...
const queryKey = computed(() => [
  "dead-letters", page.value, pageSize.value, filterFailureClass.value, filterReplayState.value,
] as const);
  ...
  <div>
    ...
    <div class="card mb-6 p-4">
      <div class="flex flex-wrap items-center gap-3">
        <select v-model="filterFailureClass"
        ...
        <select v-model="filterReplayState"
          class="rounded-lg border border-surface-200 bg-white px-3 py-2 text-sm text-surface-700 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500">
        ...
        <span class="text-xs text-surface-400">{{ data?.page_info?.total_items ?? 0 }} 条死信</span>
      </div>
    </div>
    ...
    <DataTable
      ...
      :loading="isLoading"
      empty-message="无死信记录"
      clickable row-key="dead_letter_id"
    ...
    >
      ...
      <template #cell-error_message="{ value }">
        <span class="text-xs text-surface-400 truncate block max-w-[200px]">{{ value || "—" }}</span>
      </template>
    </DataTable>
    ...
    <DetailDrawer :open="drawerOpen" title="死信详情" @close="drawerOpen = false">
      <template v-if="selectedItem">
        <dl class="space-y-4">
          ...
          <div>
            ...
            <dd class="mt-1"><StatusBadge :status="selectedItem.failure_class" /></dd>
          </div>
          <div>
            <dt class="text-2xs font-semibold uppercase text-surface-400">重放状态</dt>
            <dd class="mt-1"><StatusBadge :status="selectedItem.replay_state" /></dd>
          </div>
          <div>
            <dt class="text-2xs font-semibold uppercase text-surface-400">错误信息</dt>
            <dd class="mt-1 font-mono text-xs text-red-600 whitespace-pre-wrap">{{ selectedItem.error_message || "—" }}</dd>
          </div>
          <div>
      ...
      <template v-if="selectedItem && canReplay(selectedItem.replay_state)" #footer>
        <button class="btn btn-primary w-full" :disabled="replayMutation.isPending.value" @click="handleReplay">
          {{ replayMutation.isPending.value ? "提交中..." : "提交审核重放" }}
        </button>
      </template>
    </DetailDrawer>
  </div>
