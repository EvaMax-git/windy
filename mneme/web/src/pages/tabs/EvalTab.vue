...
const listKey = computed(() => [
  "eval-tasks", page.value, pageSize.value, filterStatus.value, filterTaskType.value,
] as const);
...
<template>
  <div>
    <!-- Create task toggle -->
    <div class="mb-6">
      <button v-if="!showNewForm" class="btn btn-primary btn-sm" @click="showNewForm = true">
        ...
        <div class="space-y-3">
          <div class="grid grid-cols-2 gap-3">
            <div class="flex flex-col gap-1">
              ...
              <input v-model="newTask.task_name" type="text" placeholder="例如: 记忆提取评估 v1"
          ...
          <div class="flex flex-col gap-1">
            ...
            <input v-model="newTask.description" type="text" placeholder="任务描述..."
              class="rounded-lg border border-surface-200 bg-white px-3 py-2 text-sm text-surface-700 placeholder-surface-400 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" />
          </div>
          <div class="flex items-center gap-2">
            <button class="btn btn-primary btn-sm" :disabled="creating || !newTask.task_name.trim()" @click="handleCreateTask">
              {{ creating ? '创建中...' : '创建任务' }}
            </button>
    ...
    <div class="card mb-6 p-4">
      <div class="flex flex-wrap items-end gap-3">
        ...
        <button class="btn btn-ghost btn-sm" @click="filterStatus = ''; filterTaskType = ''; page = 1">清除</button>
        <span class="text-xs text-surface-400 ml-auto self-end pb-1">{{ data?.page_info?.total_items ?? 0 }} 个任务</span>
      </div>
    </div>
    ...
    <DataTable
      ...
      clickable row-key="task_id"
      @row-click="openDetail"
    >
      <template #cell-task_name="{ value, item }">
        <div class="max-w-[200px]">
          <p class="truncate text-sm font-medium text-surface-800">{{ value }}</p>
          <p v-if="(item as EvalTask).description" class="text-2xs text-surface-400 truncate mt-0.5">
            {{ (item as EvalTask).description }}
          </p>
        </div>
      </template>
      <template #cell-task_type="{ value }">
        <span :class="['badge text-2xs font-medium', TASK_TYPE_STYLES[value as string] || 'bg-surface-100 text-surface-600']">
        ...
        <div class="flex items-center gap-2 min-w-[100px]">
          <div class="flex-1 h-1.5 bg-surface-200 rounded-full overflow-hidden">
            <div class="h-full rounded-full transition-all duration-300"
              ...
              :style="{ width: formatProgress(item as EvalTask) }"></div>
          </div>
          <span class="text-2xs font-mono text-surface-500 shrink-0">{{ (item as EvalTask).processed_items }}/{{ (item as EvalTask).total_items }}</span>
        </div>
      </template>
    ...
    <!-- Detail Drawer -->
    <DetailDrawer :open="drawerOpen" title="评估任务详情" width="w-[580px] max-w-full"
      ...
      <template v-else-if="taskDetail">
        ...
        <div class="flex items-center gap-1 border-b border-surface-200 mb-4 -mx-5 px-5">
          <button v-for="tab in [
            ...
            :class="['pb-2.5 px-3 text-sm font-medium border-b-2 transition-colors',
              drawerTab === tab.key ? 'border-brand-500 text-brand-600' : 'border-transparent text-surface-400 hover:text-surface-600']">
            {{ tab.label }}
          </button>
        </div>
          ...
          <dl class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <dd class="mt-1">
                  <span :class="['badge text-2xs font-medium', TASK_TYPE_STYLES[taskDetail.task_type] || 'bg-surface-100 text-surface-600']">
                ...
                <dd class="mt-1">
                  <div class="flex-1 h-2 bg-surface-200 rounded-full overflow-hidden">
                    <div class="h-full rounded-full"
                      ...
                      :style="{ width: formatProgress(taskDetail) }"></div>
                  </div>
                  <span class="text-xs font-mono text-surface-500">{{ taskDetail.processed_items }}/{{ taskDetail.total_items }}</span>
                </dd>
              </div>
            </div>
            ...
            <div v-if="taskDetail.config && Object.keys(taskDetail.config).length > 0">
              ...
              <div class="rounded-lg border border-surface-200 divide-y divide-surface-100 overflow-hidden">
                <div v-for="(val, key) in taskDetail.config" :key="key" class="flex items-start gap-3 px-3 py-2">
            ...
            <div v-if="taskDetail.error_message">
              ...
              <dd class="mt-1 text-sm text-red-600 bg-red-50 rounded p-2 font-mono text-xs break-all">{{ taskDetail.error_message }}</dd>
            </div>
          </dl>
          <div class="mt-6 pt-4 border-t border-surface-200 flex items-center gap-2">
            <button v-if="taskDetail.status === 'pending'" class="btn btn-primary btn-sm" :disabled="actionLoading" @click="handleRunTask(taskDetail.task_id)">运行评估</button>
            <button v-if="taskDetail.status === 'running'" class="btn btn-danger btn-sm" :disabled="actionLoading" @click="handleCancelTask(taskDetail.task_id)">取消运行</button>
          </div>
        </div>
        ...
        <div v-if="drawerTab === 'metrics'">
          <div v-if="overviewMetrics.length === 0" class="py-8 text-center text-sm text-surface-400">
            <div class="flex flex-col items-center gap-2"><span>暂无指标 — 请先运行评估任务</span></div>
          </div>
          <div v-else class="space-y-3">
            <div v-for="metric in overviewMetrics" :key="metric.metric_name + metric.aggregation"
              ...
              <div class="flex items-center justify-between mb-3">
                ...
                <span class="badge text-2xs bg-surface-100 text-surface-500">{{ metric.aggregation }}</span>
              </div>
              <span class="text-lg font-bold text-surface-800 font-mono">{{ metricValueDisplay(metric.value) }}</span>
              <div class="h-2 bg-surface-100 rounded-full overflow-hidden mb-2">
                <div class="h-full rounded-full bg-brand-500" :style="{ width: `${Math.min(100, metric.value * (metric.value <= 1 ? 100 : 1))}%` }"></div>
              </div>
        ...
        <div v-if="drawerTab === 'results'">
          <div v-if="!taskResults || taskResults.items.length === 0" class="py-8 text-center text-sm text-surface-400">
            <div class="flex flex-col items-center gap-2"><span>暂无评估结果</span></div>
          </div>
          <div v-else class="space-y-3">
            <div v-for="result in taskResults.items" :key="result.result_id"
              class="rounded-lg border border-surface-200 p-3 hover:border-surface-300 transition-colors">
              <div class="flex items-center justify-between mb-2">
                <span class="text-2xs font-semibold uppercase text-surface-400">#{{ result.item_index }}</span>
                <span class="font-mono text-2xs text-surface-400">{{ result.result_id.slice(0, 8) }}...</span>
              </div>
              ...
              <div v-if="result.metrics && Object.keys(result.metrics).length > 0" class="flex flex-wrap gap-2">
                <span v-for="(val, key) in result.metrics" :key="key" class="badge text-2xs font-mono bg-brand-50 text-brand-700">
      ...
      <div v-else class="flex flex-col items-center justify-center py-12 text-sm text-surface-400">加载评估详情失败</div>

      <template #footer>
        <div class="flex items-center justify-between">
          <span class="text-2xs text-surface-400">ID: {{ selectedTaskId ? selectedTaskId.slice(0, 8) + '...' : '—' }}</span>
          <button class="btn btn-secondary btn-sm" @click="drawerOpen = false; selectedTaskId = null">关闭</button>
        </div>
      </template>
    </DetailDrawer>
  </div>
