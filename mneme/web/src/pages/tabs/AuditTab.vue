...
const queryKey = computed(() => [
  "audit-events", page.value, pageSize.value, filterActorType.value,
  filterAction.value, filterResult.value, filterDateAfter.value, filterDateBefore.value,
] as const);
    ...
    <div class="card mb-6 p-4">
      <div class="flex flex-wrap items-end gap-3">
        ...
        <div class="flex flex-col gap-1">
          ...
          <input v-model="filterAction" type="text" placeholder="e.g. review.approved..."
        ...
        <div class="flex flex-col gap-1">
          ...
          <input v-model="filterDateAfter" type="datetime-local"
            class="rounded-lg border border-surface-200 bg-white px-3 py-2 text-sm text-surface-700 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" />
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-2xs font-semibold uppercase text-surface-400">至</label>
          <input v-model="filterDateBefore" type="datetime-local"
            class="rounded-lg border border-surface-200 bg-white px-3 py-2 text-sm text-surface-700 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" />
        </div>
        <button class="btn btn-ghost btn-sm" @click="clearFilters">清除</button>
        <span class="text-xs text-surface-400 ml-auto self-end pb-1">{{ data?.page_info?.total_items ?? 0 }} 条事件</span>
      </div>
    </div>
        ...
        <dl class="space-y-4">
          ...
          <div>
            ...
            <dd class="mt-1 text-sm">
              <StatusBadge :status="selectedItem.actor?.actor_type || ''" :label="selectedItem.actor?.actor_type || '—'" />
            </dd>
          ...
          <div>
            ...
            <dd class="mt-1 text-sm">
              ...
              <span v-if="selectedItem.object_id" class="font-mono text-xs text-surface-400">/ {{ selectedItem.object_id.slice(0, 12) }}...</span>
            </dd>
          </div>
          <div>
            <dt class="text-2xs font-semibold uppercase text-surface-400">结果</dt>
            <dd class="mt-1"><StatusBadge :status="selectedItem.result" /></dd>
          </div>
          ...
          <div>
            ...
            <dd class="mt-1"><StatusBadge :status="selectedItem.sensitivity_level" :label="selectedItem.sensitivity_level" /></dd>
          </div>
          <div>
